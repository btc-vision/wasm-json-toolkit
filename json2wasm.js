...
_exports.immediataryGenerators = {
    ...
    'memory_index': (json, stream) => {
        leb.unsigned.write(json.index, stream);
        return stream;
    },
    'memory_copy_immediate': (json, stream) => {
        leb.unsigned.write(json.destMemoryIndex, stream);
        leb.unsigned.write(json.sourceMemoryIndex, stream);
        return stream;
    },
    'table_fill_immediate': (json, stream) => {
        leb.unsigned.write(json.index, stream);
        // Handle other fields if necessary
        return stream;
    },
    'call_indirect_immediate': (json, stream) => {
        leb.unsigned.write(json.typeIndex, stream);
        stream.write([json.reserved]);
        return stream;
    }
}
...
_exports.generateOp = (json, stream = new Stream()) => {
    ...
    if (json.return_type !== undefined) {
        // Handle specific type const instructions
    ...
    if (opcode === undefined) {
        throw new Error(`Unsupported opcode name: ${name}`);
    }

    if (opcode >= 0xFC00) {
        // Handle multibyte opcodes
        stream.write([0xFC, opcode & 0xFF]);
    } else {
        stream.write([opcode]);
    }
    ...
    if (immediates) {
        if (!_exports.immediataryGenerators[immediates]) {
            throw new Error(`Unsupported generator for ${immediates} - ${json.name} - ${json.return_type}`);
        }
