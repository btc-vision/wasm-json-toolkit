...
const OPCODES = _exports.OPCODES = {
    ...
    // Multibyte instructions starting with 0xFC
...
_exports.immediataryParsers = {
    ...
    'memory_copy_immediate': (stream) => {
        const json = {};
        json.destMemoryIndex = leb.unsigned.readBn(stream).toNumber();
        json.sourceMemoryIndex = leb.unsigned.readBn(stream).toNumber();
        return json;
    },
    'table_fill_immediate': (stream) => {
        const json = {};
        json.index = leb.unsigned.readBn(stream).toNumber();
        return json;
    },
    'call_indirect_immediate': (stream) => {
        const json = {};
        json.typeIndex = leb.unsigned.readBn(stream).toNumber();
        json.reserved = stream.read(1)[0];
        return json;
    }
};
...
_exports.parseOp = (stream) => {
    ...
    if (op === 0xFC) {
        // Read the next byte to form the multibyte opcode
        const extendedOp = (op << 8) | stream.read(1)[0];
        const fullName = OPCODES[extendedOp];

        if (fullName === undefined) {
            throw new Error(`Unsupported multibyte opcode ${extendedOp.toString(16)}`);
        }

        json.name = fullName;

        const immediates = OP_IMMEDIATES[fullName];
        if (immediates) {
        ...
        if (fullName === undefined) {
            throw new Error(`Unsupported opcode ${op.toString(16)}`);
        }
